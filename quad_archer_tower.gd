...
@onready var animated_sprite4: AnimatedSprite2D = $AnimatedSprite4
...
@onready var enemy_detector_4: Area2D = $AnimatedSprite4/enemyDetector4
...
# Archer 4 state
var archer4_is_attacking := false
var archer4_can_attack := true
var archer4_attack_timer := 0
var archer4_has_shot := false

func _ready() -> void:
	animated_sprite1.play("idle")
	animated_sprite2.play("idle")
	animated_sprite3.play("idle")
	animated_sprite4.play("idle")
...
func process_archer4():
	update_archer_timer(4)
	var has_enemies = get_enemies_detected(enemy_detector_4).size() > 0
	
	if has_enemies:
		if not archer4_is_attacking and archer4_can_attack:
			archer4_is_attacking = true
			archer4_has_shot = false
			animated_sprite4.play("shoot")
		elif not archer4_is_attacking:
			animated_sprite4.play("idle")
	else:
		if not archer4_is_attacking:
			animated_sprite4.play("idle")
...
func update_archer_timer(archer_num: int):
	if archer_num == 1:
		...
		if not archer4_can_attack:
			archer4_attack_timer -= 1
			if archer4_attack_timer <= 0:
				archer4_can_attack = true
...
func _on_archer4_animation_finished() -> void:
	if animated_sprite4.animation == "shoot":
		animated_sprite4.play("idle")
		archer4_is_attacking = false
		archer4_can_attack = false
		archer4_attack_timer = randi_range(8, 18)
...
func _on_animated_sprite_2d_frame_changed() -> void:
	...
	if animated_sprite4.animation == "shoot" and animated_sprite4.frame == 5 and not archer4_has_shot:
		archer4_has_shot = true
		shoot_arrow($AnimatedSprite4/shootLocation4, enemy_detector_4, animated_sprite4)
