...
const ARCHER_TOWER = preload("res://archer_tower.tscn")
const TWIN_ARCHER_TOWER = preload("res://twin_archer_tower.tscn")
@onready var tower_construction: Sprite2D = $TowerConstruction
...
func _process(delta: float) -> void:
	# Only do manual check if Singleton.game exists
	...
	if is_hovering and not label.visible:
		if state == "not_built":
			var cost = Singleton.game.tower_cost["archer_tower"]
			if Singleton.game.can_afford_tower("archer_tower"):
				label.text = "Build: " + str(cost) + "g"
			else:
				label.text = "Build: " + str(cost) + "g (Not enough!)"
		elif state == "level_1":
			var cost = Singleton.game.tower_cost["twin_archer_tower"]
			if Singleton.game.can_afford_tower("twin_archer_tower"):
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		if state == "not_built" and area_construction.monitoring:
			if _is_point_in_area(mouse_pos, area_construction):
				if Singleton.game.can_afford_tower("archer_tower"):
					if Singleton.game.purchase_tower("archer_tower"):
						current_archer_tower = ARCHER_TOWER.instantiate()
						current_archer_tower.position = global_position
						get_parent().add_child(current_archer_tower)
						state = "level_1"
						var upgrade_cost = Singleton.game.tower_cost["twin_archer_tower"]
		...
		# Check tower area
			...
			if _is_point_in_area(mouse_pos, area_tower):
				if Singleton.game.can_afford_tower("twin_archer_tower"):
					if Singleton.game.purchase_tower("twin_archer_tower"):
						current_archer_tower.queue_free()
						current_archer_tower = TWIN_ARCHER_TOWER.instantiate()
						current_archer_tower.position = global_position
						get_parent().add_child(current_archer_tower)
						state = "level_2"
						label.visible = false
